defmodule Harmont.GhApp.CannedRenderBackend do
  @moduledoc """
  Cross-process-safe `HarmontVm.Backend` stub for the git-event fan-out render
  path.

  A copy of the engine's `Harmont.CannedRenderBackend` (that module lives in
  the engine app's `test/support`, which is not compiled for this app's test
  suite). Reads its canned per-exec results from application env at `exec/2`
  time, so it is safe even when the render runs in a different process than the
  test. Configure with:
